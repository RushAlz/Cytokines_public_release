---
title: "Replication - Linear and/or logistic regression - Mount Sinai Brain Bank (MSBB) dataset"
author: |
...
exprData_MSBB <- read_tsv("C:/Users/beker/OneDrive/Documentos/Mestrado/GitHub/Cytokines/replication/T026/MSBB_Normalized_counts_CQN.tsv")
```
...
pheno_sel = pheno_temp[pheno_temp$tissue == "frontal pole", ] # select the frontal pole (BA = 10)

# expression matrix 
rownames(exprData_MSBB) = exprData_MSBB$feature
exprData_MSBB_sel <- exprData_MSBB[, c("feature", colnames(exprData_MSBB)[colnames(exprData_MSBB) %in% pheno_sel$specimenID])]

message(paste0("Number of samples: ", length(colnames(exprData_MSBB_sel)) - 1))
```

```{r gene_expression, warning=FALSE,message=FALSE, include=FALSE}
# Filter the gene expression
names(exprData_MSBB_sel)[names(exprData_MSBB_sel) == "feature"] <- "ensembl"
expr_MSBB_filt = exprData_MSBB_sel[exprData_MSBB_sel$ensembl %in% cytokines$ensembl,]
exprData_cytokines = merge(expr_MSBB_filt, cytokines, by = "ensembl")
...
pheno_list = c("CERAD" = "gaussian", # Test from the Consortium to Establish a Registry for Alzheimer's Disease
               "Braak" = "gaussian", # AD score for progression of neurofibrillary pathology
               "CDR" = "gaussian", #  Clinical dementia rating scale
               "plaqueMean" = "gaussian") # density of neuritic plaques
...
res_test = run_module_trait_association(data4linear_reg, phenotype_dt, pheno_list, covariates = c("ageDeath","sex", "ethnicity"))
```
...
```{r heatmap, fig.height=2.15, fig.width=6.7, warning=FALSE}
# Heatmap 
to_show = colnames(res_test$matrix_pvalue)
transpose = T
...
# Saving xlsx
write.xlsx(df_regression, file = "C:/Users/beker/OneDrive/Documentos/Mestrado/GitHub/Cytokines/replication/T027/reg_MSBB_T027.xlsx", row.names = FALSE)
```
